...
init_time = time.time()
for lat_i, lat in enumerate(lai_data.lat.values[57:58]):
    strt_time = time.time()
    for lon_i , lon in enumerate(lai_data.lon.values[1011:1012]):
        model = sm.Model(mod)
        lai_w = lai_winter.GLOBMAP_LAI.sel(lon=lon, lat=lat).to_numpy()[1:-1]
        swvlall_w = swvlall_winter.SMroot.sel(lon=lon, lat=lat).to_numpy()[2:-1]
        swvlall_su = swvlall_summer.SMroot.sel(lon=lon, lat=lat).to_numpy()[2:-1]
        swvlall_sp = swvlall_spring.SMroot.sel(lon=lon, lat=lat).to_numpy()[2:-1]
        ...
        if np.isnan(lai_w).any() == True:
            ...
            swvlall_w = swvlall_winter.SMroot.sel(lon=lon, lat=lat).to_numpy()[2:-1]
            swvlall_sp = swvlall_spring.SMroot.sel(lon=lon, lat=lat).to_numpy()[1:-1]
            swvlall_su = swvlall_summer.SMroot.sel(lon=lon, lat=lat).to_numpy()[1:-1]
